	...
	public boolean handleCommand(Player player, String[] args) {
		...
		if (arena != null) {
			...
			if (!Utils.isNumber(args[1]) || Integer.parseInt(args[1]) <= 0) {
				Messages.sendMessage(player, "&c RegenerationDelay must be a positive integer");
				return true;
			}
			arena.getStructureManager().setRegenerationDelay(Integer.parseInt(args[1]));
			Messages.sendMessage(player, "&7 Arena &6" + args[0] + "&7 RegenerationDelay set to &6" + args[1] + "&7 ticks");
