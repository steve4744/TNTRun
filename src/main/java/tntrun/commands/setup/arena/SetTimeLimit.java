	...
	public boolean handleCommand(Player player, String[] args) {
		...
		if (arena != null) {
			...
			if (!Utils.isNumber(args[1]) || Integer.parseInt(args[1]) < 0) {
				player.sendMessage("§7[§6TNTRun§7] §cTime limit must be a positive integer");
				return true;
			}
			arena.getStructureManager().setTimeLimit(Integer.parseInt(args[1]));
			player.sendMessage("§7[§6TNTRun§7] §7Arena §6" + args[0] + "§7 Time limit set to §6" + args[1] + "§7 seconds");
